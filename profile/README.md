...
Esta organização foi criada para centralizar todos os repositórios relacionados ao projeto, mantendo uma estrutura modular e organizada. Cada repositório possui uma responsabilidade específica, como documentação, desenvolvimento do backend, frontend, infraestrutura, automações (CI/CD) e demais componentes que compõem o sistema.

Este repositório serve como ponto de entrada da organização, reunindo informações gerais sobre o projeto, sua estrutura e a navegação entre os demais repositórios. As instruções específicas de configuração, instalação e execução de cada componente podem ser encontradas nos respectivos repositórios, assim como documentação e requisitos.
